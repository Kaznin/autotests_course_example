...
class PersonInfo:
    ...
    def short_name(self):
        ...
        full_name_to_list = self.full_name.split(' ')

        return f'{full_name_to_list[1]} {(full_name_to_list[0])[0]}.'
    ...
    def path_deps(self):
        ...
        for i in self.path:
            list_for_path.append(i)

        '-->'.join(list_for_path)

        return ' --> '.join(list_for_path)


    def new_salary(self):

        path_to_str = ''.join(self.path)
        dict_for_path = {}

        for i in path_to_str:
            if i not in dict_for_path:
                a = path_to_str.count(i)
                dict_for_path[i] = a
            else:
                continue
        ...
        for j in dict_for_path:
            list_for_value.append(dict_for_path[j])

        list_for_value.sort(reverse=1)

        get_sum = sum(list_for_value[:3])

        return 1337 * self.age * get_sum
