...
class Segment:
    ...
    def length(self):
        d = math.sqrt(((self.point1[0] - self.point2[0]) ** 2) + ((self.point1[1] - self.point2[1]) ** 2))
        return round(d, 2)

    def x_axis_intersection(self):
        if (self.point1[1] < 0 and self.point2[1] > 0) or (self.point1[1] > 0 and self.point2[1] < 0):
            return True
        else:
            return False

    def y_axis_intersection(self):
        if (self.point1[0] < 0 and self.point2[0] > 0) or (self.point1[0] > 0 and self.point2[0] < 0):
            return True
        else:
            return False
