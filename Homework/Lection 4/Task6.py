...
def create_phone_number(num_tuple):
    ...
    code = ''.join(map(str, new_list[:3]))
    middle_part_of_number = ''.join(map(str, new_list[3:6]))
    last_part_of_number = ''.join(map(str, new_list[6:]))

    str_phone = f'({code}) {middle_part_of_number}-{last_part_of_number}'
